...
class FilterManager {
  static Future<img.Image> applyFilter(
      ApplyFilterParams applyFilterParams) async {
    ...
    for (int y = 0; y < applyFilterParams.img.height; y++) {
      for (int x = 0; x < applyFilterParams.img.width; x++) {
        ...
        final newColor = _multiplyByColorFilter(
            [red, green, blue, alpha], applyFilterParams.colorMatrix);

        final color =
            img.ColorRgba8(newColor[0], newColor[1], newColor[2], newColor[3]);
        applyFilterParams.img.setPixel(x, y, color);
      }
    }

    return await applyFilterParams.img;
  }

  static List<int> _multiplyByColorFilter(
      List<num> color, List<double> matrix) {
